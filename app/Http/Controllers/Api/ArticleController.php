...
class ArticleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $paginationData = $articles->toArray();
    ...
    public function similar($id)
    {
        ...
        $similarArticles = Article::where('category_id', $articles->category_id)
            ...
            ->inRandomOrder()
            ->get();
        $data = new ArticleCollection( $similarArticles);
        return response()->json($data);
    }
    ...
    public function getArticlesByCategory(Category $category)
    {
        ...
        // $data = $articles->whereNull('Buyer');
        // return ArticleResource::collection($data);

        if ($category->id === 1) {
            // Si c'est la première catégorie, récupérer tous les articles
            $articles = \App\Models\Article::all();
        } else {
            // Sinon, récupérer les articles de la catégorie sélectionnée
            $articles = $category->articles;
        }

        return ArticleResource::collection($articles->whereNull('Buyer'));
    }
